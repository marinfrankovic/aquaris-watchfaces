(() => {
  ...
  detail.innerHTML = `<a class="back-link" href="index.html">&#8592; Collection</a>
    ...
    <div class="detail-overview"><figure class="watch-gallery"><div class="gallery-image" id="gallery-image">${imageMarkup(watch.images[0])}</div><div class="image-switch" role="group" aria-label="Display mode">${watch.images.map((image, index) => `<button type="button" data-image="${index}" aria-pressed="${index === 0}">${escape(image.label)}</button>`).join('')}</div><figcaption id="image-caption">${escape(watch.images[0].caption)}</figcaption></figure><div class="overview-copy"><p class="eyebrow">The design</p><h2>Analog character.<br>Everyday readings.</h2><p>${escape(watch.description)}</p><dl class="specs"><div><dt>Series</dt><dd>${escape(watch.series)}</dd></div><div><dt>Display</dt><dd>${escape(watch.resolution)} AMOLED</dd></div><div><dt>Time format</dt><dd>24-hour digital + local analog</dd></div><div><dt>Distribution</dt><dd>${watch.download ? 'Release PRG / USB sideload' : 'Showcase only'}</dd></div></dl><a class="text-link" href="?view=installation">Installation guide <span aria-hidden="true">&#8599;</span></a></div></div>
    ...
    ${watch.compatibilityAssessment ? `<section class="compatibility-assessment" aria-labelledby="assessment-title"><h2 id="assessment-title">Compatibility test results</h2><p class="assessment-note">Simulator passes do not mean supported downloads. Tested ${escape(watch.compatibilityAssessment.date)}.</p><details class="assessment-details"><summary>${watch.compatibilityAssessment.results.length} models tested / ${watch.compatibilityAssessment.results.filter(result => result[2] !== 'Blocked').length} passed / ${watch.compatibilityAssessment.results.filter(result => result[2] === 'Blocked').length} blocked</summary><div class="assessment-results">${watch.compatibilityAssessment.results.map(([model, display, status, reason]) => `<div class="assessment-row"><span class="assessment-model">${escape(model)}<small>${escape(display)}</small></span><strong>${escape(status)}</strong><details class="assessment-reason"><summary>Details<span class="visually-hidden"> for ${escape(model)}</span></summary><p>${escape(reason)}</p></details></div>`).join('')}</div><details class="assessment-method"><summary>Test scope and limitations</summary><p>${escape(watch.compatibilityAssessment.method)}</p></details></details></section>` : ''}
