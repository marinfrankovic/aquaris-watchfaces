(() => {
  ...
  detail.innerHTML = `<a class="back-link" href="index.html">&#8592; Collection</a>
    ...
    ${watch.compatibilityAssessment ? `<section class="compatibility-assessment" aria-labelledby="assessment-title"><p class="eyebrow">Tested ${escape(watch.compatibilityAssessment.date)}</p><h2 id="assessment-title">Compatibility test results</h2><p>${escape(watch.compatibilityAssessment.method)}</p><div class="assessment-results">${watch.compatibilityAssessment.results.map(([model, display, status, reason]) => `<article class="assessment-row"><div><h3>${escape(model)}</h3><p>${escape(display)}</p></div><div><strong>${escape(status)}</strong><p>${escape(reason)}</p></div></article>`).join('')}</div></section>` : ''}
