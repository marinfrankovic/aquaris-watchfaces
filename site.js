(() => {
  ...
  container.innerHTML = catalog.map(watch => `<article class="watch-entry">
    ...
    <div class="watch-summary"><p class="eyebrow">${escape(watch.category)} <span class="red-dot" aria-hidden="true"></span> ${escape(watch.status)}</p><h2><a href="?face=${encodeURIComponent(watch.id)}">${escape(watch.name)}</a></h2><p class="tagline">${escape(watch.tagline)}</p><p>${escape(watch.description)}</p><ul class="highlights">${watch.highlights.map(item => `<li>${escape(item)}</li>`).join('')}</ul><p class="device">${escape(watch.device)}</p><a class="text-link" href="?face=${encodeURIComponent(watch.id)}">Explore ${escape(watch.series)} <span aria-hidden="true">&#8599;</span></a></div>
  </article>`).join('');
