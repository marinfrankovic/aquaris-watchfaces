(() => {
  ...
  const mediaMarkup = media => media.type === 'video'
    ? `<video controls playsinline preload="metadata" poster="${escape(media.poster)}" aria-label="${escape(media.alt)}"><source src="${escape(media.src)}" type="video/mp4">Your browser does not support video playback. <a href="${escape(media.src)}">Download video</a></video>`
    : imageMarkup(media);
  ...
  collection.hidden = true;
  detail.hidden = false;
  const gallery = [...watch.images, ...(watch.videos || [])];
  ...
  detail.innerHTML = `<a class="back-link" href="index.html">&#8592; Collection</a>
    ...
    <div class="detail-overview"><figure class="watch-gallery"><div class="gallery-image" id="gallery-image">${mediaMarkup(gallery[0])}</div><div class="image-switch" role="group" aria-label="Watch gallery">${gallery.map((media, index) => `<button type="button" data-image="${index}" aria-pressed="${index === 0}">${escape(media.label)}</button>`).join('')}</div><figcaption id="image-caption">${escape(gallery[0].caption)}</figcaption></figure><div class="overview-copy"><p class="eyebrow">The design</p><h2>Analog character.<br>Everyday readings.</h2><p>${escape(watch.description)}</p><dl class="specs"><div><dt>Series</dt><dd>${escape(watch.series)}</dd></div><div><dt>Display</dt><dd>${escape(watch.resolution)} AMOLED</dd></div><div><dt>Time format</dt><dd>${escape(watch.timeFormat || '24-hour digital + local analog')}</dd></div><div><dt>Distribution</dt><dd>${watch.download ? 'Release PRG / USB sideload' : 'Showcase only'}</dd></div></dl><a class="text-link" href="?view=installation">Installation guide <span aria-hidden="true">&#8599;</span></a></div></div>
  ...
  detail.querySelectorAll('[data-image]').forEach(button => button.addEventListener('click', () => {
    ...
    const media = gallery[Number(button.dataset.image)];
    const galleryElement = document.querySelector('#gallery-image');
    const video = galleryElement.querySelector('video');
    if (video) video.pause();
    galleryElement.innerHTML = mediaMarkup(media);
    document.querySelector('#image-caption').textContent = media.caption;
